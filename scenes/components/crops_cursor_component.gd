...
func _unhandled_input(event: InputEvent) -> void:
	if event.is_action_pressed("remove_dirt"):
		if ToolManager.selected_tool== Data_types.Tools.TillGround:
			get_cell_under_mouse()
			remove_crop()

	elif event.is_action_pressed("hit"):
		if ToolManager.selected_tool == Data_types.Tools.PlantCorn or Data_types.Tools.PlantTomato:
			get_cell_under_mouse()
			add_crop()
...
func add_crop() -> void:
	if distance < 20.0:
		if ToolManager.selected_tool == Data_types.Tools.PlantCorn:
		...
		if ToolManager.selected_tool == Data_types.Tools.PlantTomato:
